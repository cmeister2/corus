//! `corus` - snapshot a running process into an ELF core file.
//!
//! Two surfaces over one engine ([`corus_core`]):
...
//! strictly-`no_std` [`corus_core`]. Unwinding never crosses the FFI
...
macro_rules! capture_here {
    () => {{
        let tid = ::corus_core::corus_syscall::sys::gettid()
            .map(|t| t as core::ffi::c_int)
            .unwrap_or(0);
        let mut regs: ::corus_core::elf::Regs = unsafe { core::mem::zeroed() };
        unsafe {
            ::corus_core::corus_syscall::arch::capture_frame(
                &mut regs as *mut ::corus_core::elf::Regs as *mut u64,
            )
        };
        Some((tid, regs))
    }};
