...
fn dump_from_deep_frame(path: &Path) -> Result<(), corus::Error> {
    let marker = 0xC0DE_D00D_FEED_FACE_u64;
    std::hint::black_box(marker);

    unsafe { corus::CoreDump::builder().write_to_path(path) }
}

#[inline(never)]
/// Middle frame kept visible in debugger backtraces.
fn call_level_two(path: &Path) -> Result<(), corus::Error> {
    dump_from_deep_frame(path)
}

#[inline(never)]
/// Outermost helper frame kept visible in debugger backtraces.
fn call_level_one(path: &Path) -> Result<(), corus::Error> {
    call_level_two(path)
}
