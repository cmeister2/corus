...
//!
//! This inspects the compiled `chorus-core` rlib with `nm` and fails if any
...
fn engine_rlib_has_no_alloc_or_lock_symbols() {
    ...
    // Find our own rlib under target/.../deps/libchorus_core-*.rlib.
    ...
    let rlib = std::fs::read_dir(&dir).ok().and_then(|entries| {
        entries.flatten().map(|e| e.path()).find(|p| {
            ...
            n.starts_with("libchorus_core-") && n.ends_with(".rlib")
        })
    });
    let rlib = match rlib {
        Some(p) => p,
        None => {
            eprintln!("skipping: chorus_core rlib not found in {dir:?}");
            return;
        }
    };
    ...
    assert!(
        hits.is_empty(),
        "chorus-core (the post-suspension engine) must not reference libc \
