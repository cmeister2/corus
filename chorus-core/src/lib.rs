//! `chorus-core` - the `no_std` core dump engine.
//!
...
pub unsafe fn write_core_dump_to_fd_options(
    ...
    if opts_owned.frame.is_none() {
        let tid = chorus_syscall::sys::gettid()
            .map(|t| t as c_int)
            .unwrap_or(0);
        let mut regs: elf::Regs = unsafe { mem::zeroed() };
        unsafe { chorus_syscall::arch::capture_frame(&mut regs as *mut elf::Regs as *mut u64) };
...
extern "C" fn dump_callback(param: *mut c_void, pids: *const c_int, num: c_int) -> c_int {
    ...
    let main_pid = chorus_syscall::sys::getpid()
        .map(|p| p as c_int)
