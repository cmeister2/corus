...
//! `chorus_core` engine plus the compression pipeline.
//!
...
use chorus_core::chorus_syscall::linux::EINVAL;
use chorus_core::dump::{DumpOptions, ExtraNote};
use chorus_core::elf;
...
unsafe fn dump_with_params_framed(
    ...
    unsafe {
        dump_with_params_using(out_fd, params, frame, |fd, opts| {
            chorus_core::write_core_dump_to_fd_options(fd, opts).unwrap_or(-1)
        })
    }
}
...
pub extern "C" fn WriteCoreDumpLimited(file_name: *const c_char, max_length: usize) -> c_int {
    ...
    write_to_named_file(file_name, |fd| {
        ...
        unsafe { chorus_core::write_core_dump_to_fd_options(fd, &opts) }.unwrap_or(-1)
    })
}
...
unsafe fn dump_compressed(
    ...
    if compressors.is_null() {
        return unsafe { chorus_core::write_core_dump_to_fd_options(out_fd, opts) }.unwrap_or(-1);
    }
    ...
    loop {
        ...
        if name.to_bytes().is_empty() {
            let rc =
                unsafe { chorus_core::write_core_dump_to_fd_options(out_fd, opts) }.unwrap_or(-1);
            if rc == 0 {
